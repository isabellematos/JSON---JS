   ...
   if(typeof(sigla) == 'undefined')
   {
       if(sigla != '' && sigla.length == 2)
       {
          // percorrer o array de cidades para validar a sigla do estado
           city.forEach ((item) => {
       // localiza um item no array com o indexOf, retorna 0 se encontrar algo, -1 se nao escontrar.
           if(item.sigla.indexOf(sigla.toUpperCase()) == 0)
           {
              ...
              for (let i = 0 ; i < item.cidades.length; i++){
               cidadeResultado.push(item.cidades[i].nome)
               erro = false
              }
           }
       });
